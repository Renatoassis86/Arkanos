-- ============================================================
-- 0010 — Títulos: concessão (grant_title), equipar (equip_title) e a medalha
-- de trilha 'mente-clara' (90%+ no Desafio/Lógica). award_arks passa a conceder
-- títulos por marco. Mesma assinatura/retorno de 0009. Idempotente.
-- ============================================================

-- Concede um título (idempotente). Auto-equipa o PRIMEIRO título do usuário.
drop function if exists public.grant_title(text);
create function public.grant_title(p_key text)
...
begin
  ...
  -- se o usuário ainda não tem nenhum título equipado, equipa este.
  if not exists (select 1 from public.user_titles where user_id = v_uid and equipped) then
    update public.user_titles set equipped = true where user_id = v_uid and title_key = p_key;
  end if;
  return true;
end;
$$;

-- Equipa um título (e desequipa os demais) numa só operação. SECURITY DEFINER.
drop function if exists public.equip_title(text);
create function public.equip_title(p_key text)
returns void language plpgsql security definer set search_path = public as $$
declare v_uid uuid := auth.uid();
begin
  if v_uid is null then return; end if;
  update public.user_titles
     set equipped = (title_key = p_key)
   where user_id = v_uid;
end;
$$;

-- ============================================================
-- award_arks: + medalha 'mente-clara' (Desafio ≥90%) e concessão de TÍTULOS
-- por marco (silenciosos — não entram na fila de premiação).
-- ============================================================
...
begin
  ...
  -- Medalhas (entram na fila de premiação).
  ...
  -- Títulos por marco (silenciosos; grant_title é idempotente).
