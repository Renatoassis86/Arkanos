...
export function Hero({ authed }: { authed?: boolean }) {
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-[#1e3a8a] via-[#1f57d6] to-[#2f73ef] px-4 pt-28 pb-16 sm:px-6 sm:pt-32 sm:pb-20">
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl items-center gap-8 lg:grid-cols-2 lg:gap-10">
        {/* Texto */}
        <Reveal>
          <p className="font-emblem mb-4 inline-flex flex-wrap items-center gap-x-2 rounded-full border border-slate-200 bg-white px-3.5 py-1.5 text-[11px] sm:text-xs font-bold uppercase tracking-[2px] sm:tracking-[3px] shadow-sm">
          ...
          <h1 className="font-display text-3xl font-black leading-tight text-white sm:text-5xl md:text-[3.5rem]">
            Cada estudante é <span className="text-[#fcd34d]">único</span>, sua jornada de
            saber <span className="text-[#fcd34d]">também</span>.
          </h1>
          <p className="mt-4 sm:mt-6 max-w-xl text-base sm:text-lg leading-relaxed text-blue-50/90">
          ...
          <p className="mt-3 sm:mt-4 text-xs text-blue-100/80">
            {authed
              ? "Você já está conectado! Clique em Acessar Meu Painel para jogar."
              : "Os jogos abrem ao entrar. Crie sua conta para começar a jornada."}
          </p>
        </Reveal>

        {/* Foto real recortada em máscara desconstruída */}
        <Reveal delay={0.15} className="flex justify-center">
          <PhotoMask
            ...
            priority
            framePad={7}
            className="w-full max-w-[360px] sm:max-w-[480px] lg:max-w-[560px]"
          />
        </Reveal>
      </div>
    </section>
