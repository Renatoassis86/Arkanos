  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-white/10 bg-gradient-to-r from-[#1e3a8a] to-[#3730a3] text-white shadow-[0_4px_20px_rgba(2,6,23,0.25)]">
      <div className="mx-auto flex max-w-5xl items-center gap-2 px-2 py-2 sm:px-4">
        <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:justify-center">
          {ITEMS.map((it) => {
            ...
            return (
              <Link
                key={it.label}
                href={it.href}
                className={`shrink-0 rounded-full px-3 py-2 text-[11px] font-black uppercase tracking-wide transition sm:px-3.5 sm:text-xs ${
